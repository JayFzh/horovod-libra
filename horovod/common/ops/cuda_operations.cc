...
public:
  ...
  // void wairForEventsFzh(HorovodGlobalState& state,const std::function<void()>& error_check_callback){
  //     while(!state.thread_fzh_shutdown){
  //         while (!state.event_queue.empty()) {
  //           std::string name;
  //           cudaEvent_t event;
  //           std::tie(name, event) = state.event_queue.front();
  //           state.event_queue.pop();
  //           if (name != "") {
  //             state.timeline.ActivityStartAll(state.fzh_entries, name);
  //           }
  ...
  //           }
  //           if (name != "") {
  //             state.timeline.ActivityEndAll(state.fzh_entries);
  //           }
